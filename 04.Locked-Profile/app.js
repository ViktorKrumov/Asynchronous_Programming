async function lockedProfile() {
    ...
    const profilesEndpoint = `http://localhost:3030/jsonstore/advanced/profiles`;

    const profilesResponse = await fetch(profilesEndpoint);
    console.log(profilesResponse.ok);
    ...
    buttons.map(x => x.addEventListener('click', showHideFields));
